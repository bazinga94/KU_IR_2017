...
def load_data_and_labels(happy_data_file, sad_data_file,angry_data_file,calm_data_file):
    ...
    # Generate labels
    happy_labels = [[0,0,1,0] for _ in happy_examples]
    sad_labels = [[0,0,0,1] for _ in sad_examples]
    angry_labels = [[1,0,0,0] for _ in angry_examples]
    calm_labels = [[0,1,0,0] for _ in calm_examples]
    y = np.concatenate([happy_labels, sad_labels,angry_labels,calm_labels], 0)
    return [x_text, y]

def load_data_and_labels_for456(happy_data_file, sad_data_file,angry_data_file,calm_data_file):
    """
    Loads MR polarity data from files, splits the data into words and generates labels.
    Returns split sentences and labels.
    """
    ...
    x_text = [clean_str(sent) for sent in x_text]
    # Generate labels
    angry_labels= [[0,0,0,1] for _ in angry_examples]
    calm_labels= [[0,0,1,0] for _ in calm_examples]
    happy_labels = [[0,1,0,0] for _ in happy_examples]
    sad_labels = [[1,0,0,0] for _ in sad_examples]
    y = np.concatenate([happy_labels, sad_labels,angry_labels,calm_labels], 0)
    return [x_text, y]
